...
class AdminController extends Controller
{
    ...
    // Add/Update these methods in AdminController.php

    public function updateOrderStatus(Request $request, $id)
    {
        $order = Order::findOrFail($id);

        // Real-world statuses: pending -> confirmed -> out_for_delivery -> delivered
        $request->validate([
    ...
    public function cancelOrder($id)
    {
        ...
        return redirect()->back()->with('warning', 'Order has been cancelled.');
    }
    ...
    public function togglePaymentStatus($id)
    {
        $order = Order::findOrFail($id);
        if ($order->payment) {
            $newStatus = ($order->payment->status === 'paid') ? 'unpaid' : 'paid';
            $order->payment->update(['status' => $newStatus]);
        }
        return redirect()->back()->with('success', 'Payment status manually adjusted.');
    }
    public function confirmPayment($id)
    {
        ...
        // 2. Real-world logic: If money is received, the order is automatically "Confirmed"
        $order->update(['status' => 'confirmed']);

        return back()->with('success', 'Payment verified! Order is now confirmed for shipping.');
    }
    ...
    public function editOrderItems(Request $request, $id)
    {
        ...
        $newTotal = $order->items->sum(function ($i) {
            return $i->product->product_price * $i->quantity;
        });

        $order->update(['total_price' => $newTotal]);

        return back()->with('success', 'Order details and totals updated successfully!');
    }
}
